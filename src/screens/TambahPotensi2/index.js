...
class TambahPotensi2 extends React.Component {
  state = {
    avatarSource: '',
    fileName: '',
    type: '',
    ...
    data: [],
    marker: false,
    idKonten:this.props.route.params.id
  };
  componentDidMount() {
    this.getDataPotensi();
  ...
  submit = () => {
    AsyncStorage.getItem('access').then((value) => {
      ...
      const url = `https://api.istudios.id/v1/potensi/${idKonten}/`;
      if (
        fileName != '' &&
        kontak != '' &&
        judul != '' &&
