...
class SuratKeteranganBelumMenikah extends React.Component {
  state = {
    data: [],
    token: '',
    loading: false,
  ...
  tambahLayanan = () => {
    const {defaultName, pendudukID} = this.state;
    if (defaultName != 'Select ...') {
      this.setState({modalVisible: true});
      const url = 'https://api.istudios.id/v1/layanansurat/belummenikah/';

      const datas = {
        atribut: {
          orangtua_id: pendudukID,
        },
      };
      axios
        .post(url, datas, {
...
export default SuratKeteranganBelumMenikah;
